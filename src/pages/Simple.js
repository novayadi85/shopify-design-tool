...
const SimpleContent = (props) => {
                                                ...
                                                return (
                                                    <div key={ `colIndex-${colIndex}`} className={`grid-column-rows rows-index${colIndex } ${col?.className}`}>
                                                        {(col.handle === 'block-button') ? (
                                                            <div onClick={() => activateSectionBlock(col)} style={{ display: 'inline-block' }} key={columnIndex} className={`sa-content sa-block-${col.ID}`}>
                                                                
                                                            </div>
                                                        ) : (
                                                            <div onClick={() => activateSectionBlock(col)} key={`child-${colIndex}`} className={`sa-content sa-block-${col.ID} ${col.handle}`}>
    ...
    const RenderLiquidElement = ({ children }) => {
        let params = products;

        //const template = decodeHTML(ReactDOMServer.renderToString(element));
        const template = ReactDOMServer.renderToStaticMarkup(children);;
